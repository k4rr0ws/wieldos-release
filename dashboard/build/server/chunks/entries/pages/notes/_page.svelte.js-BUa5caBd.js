			...
			if (data.notes.length === 0) {
				...
				for (let $$index_3 = 0, $$length = each_array_2.length; $$index_3 < $$length; $$index_3++) {
					...
					if (Card) {
						$$renderer.push("<!--[-->");
						Card($$renderer, {
							class: `group ${note.pinned ? "ring-primary/20 ring-1" : ""}`,
							children: ($$renderer) => {
								if (Card_header) {
									$$renderer.push("<!--[-->");
									Card_header($$renderer, {
										...
										children: ($$renderer) => {
											...
											$$renderer.push(`<!----></form> <form method="POST" action="?/togglePin"><input type="hidden" name="id"${attr("value", note.id)}/> `);
											Button($$renderer, {
												type: "submit",
												variant: "ghost",
												size: "icon-sm",
								...
								if (Card_content) {
									...
									Card_content($$renderer, {
										class: "pt-0",
										children: ($$renderer) => {
											$$renderer.push(`<p class="text-muted-foreground line-clamp-4 text-sm whitespace-pre-wrap">${escape_html(note.content)}</p>`);
										},
										$$slots: { default: true }
									});
									$$renderer.push("<!--]-->");
								...
								if (Card_footer) {
									...
									Card_footer($$renderer, {
										class: "text-muted-foreground items-center justify-between gap-2 text-xs",
										children: ($$renderer) => {
											$$renderer.push(`<span class="shrink-0">Edited ${escape_html(formatRelative(note.updatedAt))}</span> `);
											if (note.projectName || note.tags.length) {
												$$renderer.push("<!--[0-->");
												$$renderer.push(`<div class="flex flex-wrap items-center justify-end gap-1.5">`);
												if (note.projectName) {
													$$renderer.push("<!--[0-->");
													Badge($$renderer, {
														variant: "outline",
...
//# sourceMappingURL=_page.svelte.js-BUa5caBd.js.map
