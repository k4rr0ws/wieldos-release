...
import { g as getProviderValues } from './instances.js-BWUEcufp.js';
import { q as queryRecords } from './records.js-CB6fc5uw.js';
import { parseAbi, parseAbiItem, isHash, isAddress, formatUnits, formatEther, decodeEventLog, createPublicClient, http, createWalletClient } from 'viem';
...
var selectBase = `
	SELECT a.id, a.type, a.label, a.provider, a.description,
		...
		a.script, a.tool_enabled AS toolEnabled,
...
function createAdapter({ type, label, provider = "", description = "", inputHint = "", defaultRecordType = "item", script = "", toolEnabled = 0 }) {
	const info = db.prepare(`INSERT INTO adapters (type, label, provider, description, input_hint, default_record_type, script, tool_enabled)
			 VALUES (?, ?, ?, ?, ?, ?, ?, ?)`).run(String(type), String(label), String(provider ?? ""), String(description ?? ""), String(inputHint ?? ""), String(defaultRecordType || "item"), String(script ?? ""), toolEnabled ? 1 : 0);
	return getAdapter(Number(info.lastInsertRowid));
}
function updateAdapter(id, { type, label, provider, description, inputHint, defaultRecordType, script, toolEnabled } = {}) {
	db.prepare(`UPDATE adapters SET
			...
			tool_enabled = COALESCE(?, tool_enabled),
			updated_at = datetime('now')
		 WHERE id = ?`).run(type != null ? String(type) : null, label != null ? String(label) : null, provider != null ? String(provider) : null, description != null ? String(description) : null, inputHint != null ? String(inputHint) : null, defaultRecordType != null ? String(defaultRecordType) : null, script != null ? String(script) : null, toolEnabled != null ? toolEnabled ? 1 : 0 : null, id);
...
async function twitterUserInfo(userName, { apiKey = "" } = {}) {
	const json = await twitterGet("/twitter/user/info", { userName }, { apiKey });
...
async function twitterTweetsByIds(ids = [], { apiKey = "" } = {}) {
	const list = (Array.isArray(ids) ? ids : [ids]).map((v) => clip(v, 32)).filter(Boolean).slice(0, MAX_ITEMS$1);
	if (!list.length) return { tweets: [] };
	return { tweets: extractTweets(await twitterGet("/twitter/tweets", { tweet_ids: list.join(",") }, { apiKey })) };
}
...
async function twitterTrends({ woeid = 1, count = 30, apiKey = "" } = {}) {
...
var dexscreener_exports = /* @__PURE__ */ __exportAll({
	...
	MAX_ITEMS: () => 12,
	dexBoosts: () => dexBoosts,
	dexFetchBoostsRaw: () => dexFetchBoostsRaw,
	dexFetchPairs: () => dexFetchPairs,
	dexFetchProfilesRaw: () => dexFetchProfilesRaw,
	dexFetchToken: () => dexFetchToken,
	dexPair: () => dexPair,
...
/** Raw pair fetch by up to 30 token contract addresses — for collect mode adapters. */
async function dexFetchToken(chainId, addresses) {
	const joined = addresses.map((a) => encodeURIComponent(a)).join(",");
	const data = await dexGet(`/tokens/v1/${encodeURIComponent(chainId)}/${joined}`);
	return (Array.isArray(data) ? data : []).map(normalizePair).sort((a, b) => (Number(b.liqUsd) || 0) - (Number(a.liqUsd) || 0)).slice(0, 12);
}
/** Raw token profiles — for collect mode adapters. */
async function dexFetchProfilesRaw() {
	return await dexGet("/token-profiles/latest/v1");
}
/** Raw boosted tokens — for collect mode adapters. */
async function dexFetchBoostsRaw(list = "top") {
	return await dexGet(list === "latest" ? "/token-boosts/latest/v1" : "/token-boosts/top/v1");
}
//#endregion
...
//#endregion
//#region src/lib/server/providers/shaper.js
function toRecord(item = {}, shape = {}) {
	...
	const id = item[shape.id || "id"];
	return {
		id: id == null ? "" : String(id),
		recordType: shape.type || "item",
		metadata,
		raw: item,
		capturedAt: shape.capturedAt ? isoOrNull(item[shape.capturedAt]) : null
	};
}
...
//#endregion
//#region src/lib/server/providers/paginate.js
/**
* @param {object} opts
* @param {(record:object)=>boolean} opts.emit Runner sink; false ⇒ stop.
...
* @param {object} [opts.shape] Record shape; items map through toRecord(item, shape).
...
*/
async function paginate({ emit, cursor = "", mode = "newest-id", fetchPage, items = (page) => page, paging = () => ({}), shape, id, build }) {
	if (!build && !shape) throw new Error("paginate needs a shape or a build(item) mapper.");
	const toRecord$1 = build ?? ((item) => toRecord(item, shape));
	const idOf = id ?? ((item) => String(item?.[shape?.id ?? "id"] ?? ""));
	const page = await fetchPage(mode === "paginate" ? cursor : "");
	const list = items(page) ?? [];
	if (mode === "snapshot") {
		for (const it of list) if (emit(toRecord$1(it)) === false) break;
		return {};
	}
	if (mode === "paginate") {
		for (const it of list) if (emit(toRecord$1(it)) === false) break;
	...
	for (const s of records) if (emit(s) === false) break;
	return { cursor: next };
}
//#endregion
...
/**
* Run one adapter against one instance. The caller owns persistence: it
* supplies `emit` and stores the returned cursor. Throws on script/compile/
* runtime errors — the caller maps those onto the collector row.
*
* @param {object} adapter  DB adapter row.
* @param {object} opts
* @param {'collect'|'fetch'} [opts.mode] 'collect' = scheduled collector (default);
*   'fetch' = agent on-demand (ctx.mode exposed to the script).
* @returns {Promise<{cursor?:string, result?:any, logs:string[]}>}
*/
async function runAdapter(adapter, { instance, credentials = {}, input = {}, cursor = "", mode = "collect", collect: emit }) {
	...
	const result = await fn({
		...
		save: (item, shape) => emit(toRecord(item, shape)),
		emit,
...
export { ERC20_ABI as E, twitterCreateTweet as a, twitterLike as b, twitterRetweet as c, twitterFollow as d, twitterSendDm as e, getPublicClient as f, getAdapterByType as g, getWalletClient as h, compileScript as i, deleteAdapter as j, getAdapter as k, listAdapters as l, createAdapter as m, runAdapter as r, str as s, toStrArray as t, updateAdapter as u };
//# sourceMappingURL=adapter-runner.js-Dsa3ucuC.js.map
