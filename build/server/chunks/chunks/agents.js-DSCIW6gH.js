...
var agentSelect = `
	SELECT a.id, a.name, a.title, a.description, a.avatar, a.status, a.persona,
		a.beliefs, a.communication_style AS communicationStyle, a.decision_style AS decisionStyle,
...
function createAgent(input) {
	const info = db.prepare(`INSERT INTO agents (
				name, title, description, avatar, status, persona, beliefs,
				communication_style, decision_style, motivations, background,
				experience, specialization, skills, strengths, tools
			) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`).run(input.name, input.title ?? "", input.description ?? "", input.avatar ?? "", input.status || "active", input.persona ?? "", input.beliefs ?? "", input.communicationStyle ?? "", input.decisionStyle ?? "", input.motivations ?? "", input.background ?? "", input.experience ?? "", input.specialization ?? "", serializeList(input.skills), serializeList(input.strengths), serializeList(input.tools));
...
function updateAgent(id, input) {
	db.prepare(`UPDATE agents SET
		 ...
		 WHERE id = ?`).run(input.name ?? null, input.title ?? null, input.description ?? null, input.avatar ?? null, input.status ?? null, input.persona ?? null, input.beliefs ?? null, input.communicationStyle ?? null, input.decisionStyle ?? null, input.motivations ?? null, input.background ?? null, input.experience ?? null, input.specialization ?? null, input.skills != null ? serializeList(input.skills) : null, input.strengths != null ? serializeList(input.strengths) : null, input.tools != null ? serializeList(input.tools) : null, id);
	return getAgent(id);
}
...
//# sourceMappingURL=agents.js-DSCIW6gH.js.map
