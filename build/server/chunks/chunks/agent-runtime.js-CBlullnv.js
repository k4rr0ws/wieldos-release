import { g as getAgent, r as recordDispatch, d as listPlaybooks, e as listRelevantObservations, f as listRelevantMemories, h as createObservation, i as dispatchAlreadyCommitted, j as recordAgentAction } from './agents.js-DSCIW6gH.js';
import { r as resolveAgentTools, t as toolSpec, g as getTool } from './tools.js-C1CBPtEY.js';
...
/**
* Build the persona preamble for an agent: who they are, how they think, what
* they remember, and the playbooks they can follow. Only non-empty fields are
* included so sparsely-filled agents still produce a clean prompt.
*
* @param {object} agent - row from getAgent()
* @param {{ memories?: Array, playbooks?: Array }} [extras]
* @returns {string}
*/
function buildAgentPersona(agent, { memories = [], observations = [], playbooks = [] } = {}) {
	...
	const traits = [
		...
		["Core beliefs", agent.beliefs],
		["Communication style", agent.communicationStyle],
		["Decision-making style", agent.decisionStyle],
	...
	if (traits.length) {
		lines.push("");
		for (const [label, value] of traits) lines.push(`${label}: ${value}`);
	}
	const lists = [
		["Skills", agent.skills],
	...
	if (lists.length) {
		lines.push("");
		for (const [label, value] of lists) lines.push(`${label}: ${value.join(", ")}`);
	}
	if (memories.length) {
	...
	lines.push("", "Act and decide in character, applying the above identity and experience.");
	return lines.join("\n");
}
/**
...
//# sourceMappingURL=agent-runtime.js-CBlullnv.js.map
