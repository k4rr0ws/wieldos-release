import { a1 as head, _ as ensure_array_like, a9 as escape_html, a3 as spread_props, a0 as attr, a6 as stringify } from '../../../chunks/server.js-BeDXxHyW.js';
...
function _page($$renderer, $$props) {
	$$renderer.component(($$renderer) => {
		...
		const typeLabel = (type) => templateForType(type)?.label ?? type;
		let $$settled = true;
		let $$inner_renderer;
							...
							children: ($$renderer) => {
								if (Card_content) {
									...
									Card_content($$renderer, {
										...
										children: ($$renderer) => {
											$$renderer.push(`<div class="min-w-0 space-y-1.5"><div class="flex items-center gap-2"><p class="truncate font-medium">${escape_html(instance.name)}</p> `);
											Badge($$renderer, {
												variant: "outline",
												children: ($$renderer) => {
													$$renderer.push(`<!---->${escape_html(typeLabel(instance.type))}`);
												},
												$$slots: { default: true }
											});
											$$renderer.push(`<!----> `);
											...
											$$renderer.push(`<!----></div> <div class="text-muted-foreground flex flex-wrap gap-x-3 gap-y-0.5 text-xs"><a${attr("href", `/collectors?instance=${stringify(instance.id)}`)} class="hover:text-foreground">${escape_html(instance.collectorCount)} collector${escape_html(instance.collectorCount === 1 ? "" : "s")}</a> <a${attr("href", `/records?instance=${stringify(instance.id)}`)} class="hover:text-foreground">${escape_html(instance.recordCount)} record${escape_html(instance.recordCount === 1 ? "" : "s")}</a> <span>${escape_html(instance.lastCheckedAt ? `checked ${formatRelative(instance.lastCheckedAt)}` : "not yet checked")}</span></div> `);
											if (Object.keys(instance.credentials).length || Object.keys(instance.config).length) {
												$$renderer.push("<!--[0-->");
												$$renderer.push(`<div class="flex flex-wrap gap-1.5 pt-0.5"><!--[-->`);
												const each_array_3 = ensure_array_like(Object.entries(instance.config));
												for (let $$index_2 = 0, $$length = each_array_3.length; $$index_2 < $$length; $$index_2++) {
													let [key, value] = each_array_3[$$index_2];
													$$renderer.push(`<span class="bg-muted text-muted-foreground rounded px-1.5 py-0.5 font-mono text-[10px]">${escape_html(key)}: ${escape_html(value)}</span>`);
												}
												$$renderer.push(`<!--]--> <!--[-->`);
												const each_array_4 = ensure_array_like(Object.entries(instance.credentials));
												for (let $$index_3 = 0, $$length = each_array_4.length; $$index_3 < $$length; $$index_3++) {
													let [key, value] = each_array_4[$$index_3];
													$$renderer.push(`<span class="bg-muted text-muted-foreground inline-flex items-center gap-1 rounded px-1.5 py-0.5 font-mono text-[10px]">`);
													Key_round($$renderer, { class: "size-3" });
													$$renderer.push(`<!---->${escape_html(key)}: ${escape_html(value)}</span>`);
												}
												$$renderer.push(`<!--]--></div>`);
											} else $$renderer.push("<!--[-1-->");
...
//# sourceMappingURL=_page.svelte.js-DBndJuOk.js.map
